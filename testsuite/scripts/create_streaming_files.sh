...
if [ $# -lt 1 ]; then
   ...
   echo Usage: `basename $0` directory  test_file_sizes number_of_files [size_unit]
   echo Example: 
...
if [ $# -lt 4 ]; then # if no unit is specified fall back to MB
    UNIT='MB'
fi
...
do
	...
	do
    	dd if=/dev/urandom of=$FOLDER/${FILE_SIZES[$i]}${UNIT}_$nr bs=1${UNIT} count=${FILE_SIZES[$i]}
	done
done
